...
def update_response = WS.sendRequest(findTestObject('Advance examples/api-2-issue/Edit issue/Edit an existing issue by Key', 
	[('issue_key') : issue_key
   , ('authorization') : GlobalVariable.authorization
   , ('summary') : str_summary]))

// Verify that the issue summary is updated correctly
def get_response = WS.sendRequest(findTestObject('Object Repository/Advance examples/api-2-issue/Get issue/Get an issue by Key', 
	[ 'issue_key' : issue_key
	, 'authorization' : GlobalVariable.authorization]))
